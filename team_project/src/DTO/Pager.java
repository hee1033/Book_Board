...
public class Pager {
	...
	public Pager(int rowsPerPage, int pagesPerGroup, int totalRows, int pageNo) {
		this.rowsPerPage = rowsPerPage; //한페이지에 나올 행 수
		this.pagesPerGroup = pagesPerGroup;//몇개의 페이지를 페이징 할건지 번호
		this.totalRows = totalRows;//페이징하고자 하는 행수
		this.pageNo = pageNo;//보고싶은 페이지 수(사용자가 입력하는 수)
